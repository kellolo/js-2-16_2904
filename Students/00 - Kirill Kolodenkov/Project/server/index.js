    ...
    fs.readFile('./server/db/catalog.json', 'utf-8', (err, data) => {
        if (!err) {
            let {
                newCatalog,
                idNewItem
            } = catalogServices.add(JSON.parse(data), req.body);
            writer('./server/db/catalog.json', newCatalog)
                .then(status => {
                    if (status) {
                        ...
                        logger('add to Catalog', req.body.product_name);
                    } else {
                        res.sendStatus(500);
                    }
                })
...
server.post('/basket/', (req, res) => {
    fs.readFile('./server/db/basket.json', 'utf-8', (err, data) => {
        if (!err) {
            let {
                newBasket
            } = basketServices.add(JSON.parse(data), req.body);
    ...
    fs.readFile('./server/db/basket.json', 'utf-8', (err, data) => {
        if (!err) {
            let {
                newBasket,
                name
            } = basketServices.change(JSON.parse(data), req.params.id, req.body.amount);
            writer('./server/db/basket.json', newBasket)
                .then(status => {
                    if (status) {
                        res.json({
    ...
    fs.readFile('./server/db/basket.json', 'utf-8', (err, data) => {
        if (!err) {
            let {
                newBasket,
                name
            } = basketServices.delete(JSON.parse(data), req.params.id);
            writer('./server/db/basket.json', newBasket)
                .then(status => {
                    if (status) {
                        res.json({
